...
export default defineConfig(({ mode }) => {
  ...
  const backendUrl = env.VITE_API_URL || 'http://localhost:5222';

  return {
    plugins: [
      react(),
      // Opt-in bundle composition report: run `ANALYZE=true npm run build`
      // to get frontend/stats.html (outside dist/, so it's never part of
      // what actually gets deployed) showing chunk sizes. Off by default
      // so ordinary builds/deploys don't spend time generating it.
